...
ρ-conf : ∀{p FV BV ql qr θ qi} → CorrectBinding p FV BV → p sn⟶₁ ql → p sn⟶₁ qr → p ≡ (ρ θ · qi)
         → ql ≡ qr
         ⊎ (∃ (λ o → ql sn⟶₁ o × qr sn⟶₁ o))
         ⊎ ql sn⟶₁ qr
         ⊎ qr sn⟶₁ ql
ρ-conf {(ρ θ · qi)}{ql = ql}{qr = qr} cb redl redr refl with ρ-stays-ρ-sn⟶₁ redl | ρ-stays-ρ-sn⟶₁ redr
... | (θl , qlin , refl) | (θr , qrin , refl) with get-view redl | get-view redr
ρ-conf {(ρ θ · qi)}{ql = ql}{qr = qr} cb redl redr refl | (θl , qlin , refl) | (θr , qrin , refl) | inj₁ (_ , _ , _ , _ , _ , viewl) | inj₁ (_ , _ , _ , _ , _ , viewr)
   with E-view-main-bind viewl viewr
... | inj₁ (e= , q= , θ=) rewrite e= | q= | θ= = inj₁ refl
... | inj₂ ~a with ρ-conf-rec cb _ _ ~a redl redr _ _ viewl viewr
... | (θo , so , _ , _ , _ , _ , _ , _ , redol , redor , _) = inj₂ $ inj₁ ((ρ θo · so) , redol , redor)
ρ-conf {(ρ θ · qi)}{ql = ql}{qr = qr} cb redl redr refl | (θl , qlin , refl) | (θr , qrin , refl) | inj₁ (_ , _ , _ , _ , _ , viewl) | inj₂ (_ , viewr)
  with ρ-pot-conf-rec cb viewr viewl
... | inj₁ redo = inj₂ (inj₂ (inj₂ redo))
... | inj₂ (s , env , redol , redor) = inj₂ $ inj₁ (ρ env · s , redor , redol)
ρ-conf {(ρ θ · qi)}{ql = ql}{qr = qr} cb redl redr refl | (θl , qlin , refl) | (θr , qrin , refl) | inj₂ (_ , viewl) | inj₁ (_ , _ , _ , _ , _ , viewr)
    with ρ-pot-conf-rec cb viewl viewr
... | inj₁ redo = inj₂ (inj₂ (inj₁ redo))
... | inj₂ (s , env , redol , redor) = inj₂ $ inj₁ ((ρ env · s) , redol , redor)
ρ-conf {(ρ θ · qi)}{ql = ql}{qr = qr} cb redl redr refl | (θl , qlin , refl) | (θr , qrin , refl) | inj₂ (_ , viewl) | inj₂ (_ , viewr)
    with pot-pot-conf-rec viewl viewr
... | inj₁ eq = inj₁ eq
... | inj₂ (env , redol , redor) = inj₂ $ inj₁ (ρ env · qi , redol , redor)
...
R-confluent {p@(ρ _ · _)} cb redl redr = ρ-conf cb redl redr refl
