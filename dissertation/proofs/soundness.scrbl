...
@proof[#:label "Soundness of guarded terms"
       #:title "Soundness of guarded terms"
       #:statement
...
@proof[#:label "soundness-of-ctx"
       #:title "Soundness of context closure"
       #:statement
       @list{For all  @es[C-pure],
        @es[p-pure] and @es[q-pure],
        if @es[(≃^circuit (compile p-pure) (compile q-pure))],
        then @es[(≃^circuit (compile (in-hole C-pure p-pure)) (compile (in-hole C-pure q-pure)))]}]{

 This proof follows direclty by induction on @es[C-pure], as
 @es[compile] is defined inductively on the same structure,
 and will add the same out circuit contexts to each term.
